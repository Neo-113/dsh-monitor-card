...
```bash
dsh plugin --profile web add file:<tgz 所在本地路径>/dsh-monitor-card-0.2.5.tgz
```
...
| `engineUrl` | `http://127.0.0.1:18080/metrics` | SGLang /metrics 地址 |
...
- 人工比对：GPU 温度 ±2°C、显存与 nvidia-smi 同值（同为 MiB，允许采样时差几十 MiB）；`host` 与 `top` 同量级；引擎与 18080 手算一致。
...
| 引擎"离线" | 18080 未运行或 /metrics 不可达；其余行不受影响 |
